...
describe('npm trusted publishing workflow contract', () => {
  test('publishes exactly the verified invompt-mcp 0.11.7 artifact through OIDC', () => {
    ...
    expect(publicPackage).toMatchObject({ name: 'invompt-mcp', version: '0.11.7' })
    expect(publicPackage.private).not.toBe(true)
    expect(workflow).toContain('- v0.11.7')
    expect(workflow).toContain("github.ref == 'refs/tags/v0.11.7'")
    expect(workflow).toContain("\n    if: github.ref == 'refs/tags/v0.11.7'")
    ...
    expect(workflow).toContain(
      'npm publish ./release-artifacts/invompt-mcp-0.11.7.tgz --access public --tag next --ignore-scripts',
    )
