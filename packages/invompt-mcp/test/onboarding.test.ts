...
describe('macOS beta onboarding', () => {
  ...
  test('has exact host commands and keeps Guest credentials out of them', () => {
    expect(hostCommands('codex', 'guest', '0.11.0')[1]).toEqual([
      'codex',
      'mcp',
      'add',
      'invompt-local-beta',
  ...
  test('preserves the injected runner contract while configuring interactive OAuth login commands', async () => {
    const commands: string[] = []
    await configureHost('claude-code', 'oauth', '0.11.0', async (command, args) => {
      commands.push([command, ...args].join(' '))
      return { ok: true }
    })
    await configureHost('codex', 'oauth', '0.11.0', async (command, args) => {
      commands.push([command, ...args].join(' '))
      return { ok: true }
    })
  ...
  test('parses required serve host and permits only narrow missing-host cleanup failures', async () => {
    ...
    await expect(
      configureHost('claude-code', 'guest', '0.11.0', async (command, args) => {
