...
describe('macOS beta onboarding', () => {
  ...
  test('has exact host commands and keeps Guest credentials out of them', () => {
    expect(hostCommands('codex', 'guest', '0.11.1')[1]).toEqual([
      'codex',
      'mcp',
      'add',
      'invompt-local-beta',
    ...
    expect(hostCommands('claude-code', 'oauth', '0.11.1')[2]).toEqual(['claude', 'mcp', 'login', 'invompt-local-beta'])
    expect(JSON.stringify(hostCommands('claude-code', 'guest', '0.11.1'))).not.toContain(credential)
    for (const host of ['claude-code', 'codex'] as const) {
      for (const mode of ['guest', 'oauth'] as const) {
        for (const command of hostCommands(host, mode, '0.11.1')) expect(command).not.toContain('invompt')
      }
    }
  })
  ...
  test('preserves the injected runner contract while configuring interactive OAuth login commands', async () => {
    const commands: string[] = []
    await configureHost('claude-code', 'oauth', '0.11.1', async (command, args) => {
      commands.push([command, ...args].join(' '))
      return { ok: true }
    })
    await configureHost('codex', 'oauth', '0.11.1', async (command, args) => {
      commands.push([command, ...args].join(' '))
      return { ok: true }
    })
  ...
  test('parses required serve host and permits only narrow missing-host cleanup failures', async () => {
    ...
    await configureHost('codex', 'guest', '0.11.1', async (_command, args) =>
    ...
    await expect(
      configureHost('claude-code', 'guest', '0.11.1', async (command, args) => {
